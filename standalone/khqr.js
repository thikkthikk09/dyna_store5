...
;(function (global) {
  ...
  /** Use relative /api/* (current site origin) — not localhost. */
  function useRelativePaymentApi() {
    if (typeof location === 'undefined') return false
    if (isGitHubPages()) return false
    if (!location.protocol.startsWith('http')) return false
    if (isLocalDev() && location.port === '8787') return true
    if (isVercelHost()) return true
    if (configApiBase() && configApiBase() === location.origin) return true
    return !isLocalDev()
  }
  ...
  function getProxyUrl() {
    ...
    const stored = (input?.value || localStorage.getItem(PROXY_KEY) || fromConfig || '').trim()
    if (stored.includes('127.0.0.1') || stored.includes('localhost')) {
      if (!isLocalDev() || location?.port !== '8787') return ''
    }
    if (stored) return stored
    if (useRelativePaymentApi()) return API_CHECK_MD5
    if (isLocalDev() && location?.port === '8787') return API_CHECK_MD5
    return `${LOCAL_PROXY_ORIGIN}${API_CHECK_MD5}`
  }
  ...
  function proxyOriginsToTry() {
    ...
    if (useRelativePaymentApi() && typeof location !== 'undefined') {
      list.push(location.origin)
    }
    if (isLocalDev() && location?.port === '8787') {
      list.push(location.origin)
    } else if (isLocalDev()) {
  ...
  function proxyBase() {
    if (useRelativeApi && typeof location !== 'undefined') return location.origin
    if (activeProxyOrigin && activeProxyOrigin !== 'direct') return activeProxyOrigin
    const base = configApiBase()
    if (base) return base
    if (useRelativePaymentApi() && typeof location !== 'undefined') return location.origin
    if (isLocalDev() && location?.port === '8787') return location.origin
    return LOCAL_PROXY_ORIGIN
  }

  /** Payment check URL — relative on Vercel/production, localhost only for local dev. */
  function resolveProxyUrl() {
    if (activeProxyOrigin === 'direct') return ''
    const raw = getProxyUrl()
    if (raw.startsWith('http') && raw.includes('/api/')) {
      useRelativeApi = false
      return raw
    }
    if (raw === API_CHECK_MD5 || (raw.startsWith('/') && raw.includes('check-md5'))) {
      useRelativeApi = true
      return raw.startsWith('/') ? raw : API_CHECK_MD5
    }
    const base =
      (activeProxyOrigin && activeProxyOrigin !== 'direct' ? activeProxyOrigin : '') ||
      configApiBase() ||
      (useRelativePaymentApi() && typeof location !== 'undefined' ? location.origin : '') ||
      (isLocalDev() && location?.port === '8787' ? location.origin : '') ||
      (isLocalDev() ? LOCAL_PROXY_ORIGIN : '')
    if (!base || base === 'direct') {
      if (useRelativePaymentApi()) {
        useRelativeApi = true
    ...
    useRelativeApi = false
    return `${String(base).replace(/\/$/, '')}${API_CHECK_MD5}`
  }

  function resolveProxyHealth() {
    if (useRelativeApi || useRelativePaymentApi()) return API_HEALTH
    return `${proxyBase().replace(/\/$/, '')}${API_HEALTH}`
  }
  ...
  async function discoverProxy() {
    ...
    for (const origin of proxyOriginsToTry()) {
      ...
      if (probe.ok) {
        markProxyOnline(origin, probe.hasJwt || hasJwtForPayment())
        return true
      }
    }
    ...
    if (useRelativePaymentApi() && typeof location !== 'undefined' && hasJwtForPayment()) {
      useRelativeApi = true
      activeProxyOrigin = location.origin
      serverHasJwt = true
  ...
  function loadSettings() {
    ...
    if (proxyEl) {
      ...
      if (stored.includes('127.0.0.1') || stored.includes('localhost')) {
        if (!isLocalDev() || location?.port !== '8787') stored = ''
      }
      if (stored === '/api/check-md5' || stored.endsWith('/check-md5')) {
        stored = useRelativePaymentApi() ? API_CHECK_MD5 : `${LOCAL_PROXY_ORIGIN}${API_CHECK_MD5}`
      }
      proxyEl.value = stored || cfg.proxy || (useRelativePaymentApi() ? API_CHECK_MD5 : '')
      if (proxyEl.value && !localStorage.getItem(PROXY_KEY)) {
